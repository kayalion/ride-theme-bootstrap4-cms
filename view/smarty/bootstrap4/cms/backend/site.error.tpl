...
{block name="taskbar_panels" append}
    {if !$site->isAutoPublish()}
        ...
        {url id="cms.site.error" parameters=["locale" => $locale, "site" => $site->getId(), "revision" => "%revision%", "node" => $node->getId()] var="url"}
        {call taskbarPanelPublish url=$url revision=$node->getRevision() revisions=$site->getRevisions()}
    {/if}

    {url id="cms.site.error" parameters=["locale" => "%locale%", "site" => $site->getId(), "revision" => $node->getRevision(), "node" => $node->getId()] var="url"}
...
{block name="content_title" append}
    {include "helper/cms/titlebar"
        node=$site
        subtitle="label.node.action.error"|translate
        breadcrumbUrl=$app.url.request
        breadcrumbLabel="label.node.action.error"|translate
        action="error"
    }
{/block}
